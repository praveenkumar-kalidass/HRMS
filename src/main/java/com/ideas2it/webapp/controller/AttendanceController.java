...
public class AttendanceController {
    ...
    /**
     * <p>
     * Mapping the request which required by user for attendance_view.html it
     * will sent the page and attendance list stored in database and model
     * object to add new user
     * </p>
     * 
     * @param model
     *            ModelMap object used for setting Attendance model object,
     ...
     * Service class to insert into the database. Processes check in for an user
     ...
     *            attendance list and User List
     * @param userId
     *            contains identity of the User it is used to many to one map
     *            with attendance
     * @return String returns the redirecting page url based on the appropriate
     *         operation.
     */
    ...
    public String createLeaveRequest(ModelMap model, final HttpServletRequest request) {
        try {
            ...
            model.addAttribute("LeaveRequestList", leaveRequestService.retriveLeaveRequests());
            model.addAttribute("OwnLeaveRequestList", leaveRequestService.retriveLeaveRequestsByUser(userId));
        } catch (DataException e) {
            model.addAttribute("message", e.getMessage());
        }
        return "leaverequest";
    ...
    public String insertLeaveRequest(@ModelAttribute("LeaveRequest") LeaveRequest leaveRequest, BindingResult result,
            ModelMap model) {
        try {
            leaveRequest.setUser(userService.searchUser(leaveRequest.getUser().getId()));
            if (leaveRequestService.addLeaveRequest(leaveRequest)) {
                model.addAttribute("message",
                        "LeaveRequest details are successfully Added Please Wait for Admin Approvel");
            } else {
