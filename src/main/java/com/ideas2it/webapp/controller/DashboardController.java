        ...
        try {
            ...
            for (User user : userService.retrieveUsers()) {
                if (user.getDesignation().getDepartment().getDepartmentName().equals("Developer")) {
                    ...
                    testingCount++;
                } else if (user.getDesignation().getDepartment().getDepartmentName().equals("HR")) {
                    hrCount++;
            ...
            long userId = 0;
            if(null != session.getAttribute("HRMSUserId")) {
                userId = (int) session.getAttribute("HRMSUserId");
                SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                Date date = format.parse(userService.searchUser(userId).getDateOfBirth());
